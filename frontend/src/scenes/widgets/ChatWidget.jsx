...
const ChatWidget = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      if (selectedCategory) {
        try {
          const response = await fetch(`http://localhost:3002/messages/${selectedCategory}`);
          const data = await response.json();
          setMessages(data);
        } catch (error) {
  ...
  const handleSend = () => {
    if (newMessage.trim() !== '' && selectedCategory) {
      ...
      socket.emit('sendMessage', { message: newMessage, userId: userId, picturePath: user.picturePath, name: user.name, category: selectedCategory });
      // console.log(newMessage,userId,user.picturePath,user.name,selectedCategory)
      setNewMessage('');
    }
  }
            ...
            return (
              <div key={index} style={{ display: 'flex', justifyContent: message.userId === userId ? 'flex-end' : 'flex-start', marginBottom: '8px' }}>
                ...
                <Typography variant="body1" sx={{ backgroundColor: message.userId === userId ? '#C8E6C9' : '#B3E5FC', borderRadius: '8px', padding: '8px 12px', color: 'black' }}>
                  {message.userId !== userId && `${message.name}:`} {message.message}
